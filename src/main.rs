...
fn main() {
    ...
    let mut out = File::create(Path::new("data/output_div.wav")).unwrap();
    ...
    let new_data: wav::BitDepth = match data {
        ...
        wav::BitDepth::Sixteen(d) => div_by_previous_16(d).into(),
        wav::BitDepth::TwentyFour(d) => div_by_previous_24(d).into(),
        wav::BitDepth::ThirtyTwoFloat(d) => div_by_previous_float(d).into(),
