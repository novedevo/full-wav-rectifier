...
the `repiquemos` library crate exposes these operations as simple scalar functions operating on floats. the `repiquemos-cli` binary crate builds them into a simple CLI that lets you process WAV files with a simple DSL to compose these operations.
